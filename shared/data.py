...
def add_stages_to_dataset(epoched_data_path, output_path, labels,
                          conditions=[], condition_variable='cue',
                          cpus=1, fit_function='fit', fit_args=dict()):
    # Check for faulty input
    if len(conditions) > 0:
        if type(labels) is dict:
            assert len(labels) == len(
                conditions), 'Not the same amount of conditions as label+conditions combinations'
        else:
            raise ValueError(
    ...
    if len(conditions) > 0:
        ...
        for condition in conditions:
            ...
            model = fit_model(condition_subset, epoch_data, cpus=cpus,
                              fit_function=fit_function, fit_args=fit_args)
...
def fit_model(hmp_data, epoch_data, cpus=1, fit_function='fit', fit_args=dict()):
    # Initialize model
    model = hmp.models.hmp(hmp_data, epoch_data,
                           cpus=cpus, sfreq=epoch_data.sfreq)
    ...
    if hasattr(model, fit_function):
        ...
        available_methods = [method_name for method_name in dir(
            model) if callable(getattr(model, method_name))]
        raise ValueError(
...
def label_model(model, eeg_data, labels):
    ...
    for locations, data in zip(event_locations, model.trial_x_participant):
        ...
        RT_sample = int(eeg_data.sel(
            participant=data[0], epochs=data[1]).isnull().argmax('samples').data[0])
        ...
        for j, location in enumerate(locations):
            ...
            samples_slice = slice(
                location, locations[j + 1]) if j != n_events - 1 else slice(location, RT_sample - 1)
