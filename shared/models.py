...
def SAT1Deep(n_channels, n_samples, n_classes):
    input = Input(shape=(n_channels, n_samples, 1))
    x = Masking(MASKING_VALUE)(input)
    x = Conv2D(filters=64, kernel_size=(1, 10), activation="relu")(x)
    # x = BatchNormalization(epsilon=1e-05, momentum=0.9)(x)
    # x = Dropout(0.25)(x)
    x = MaxPooling2D(pool_size=(1, 2))(x)
    x = Conv2D(filters=128, kernel_size=(1, 7), activation="relu")(x)
    # x = Dropout(0.25)(x)
    # x = BatchNormalization(epsilon=1e-05, momentum=0.9)(x)
    x = MaxPooling2D(pool_size=(1, 2))(x)
    x = Conv2D(filters=256, kernel_size=(1, 5), activation="relu")(x)
    # x = Dropout(0.25)(x)
    # x = BatchNormalization(epsilon=1e-05, momentum=0.9)(x)
    x = MaxPooling2D(pool_size=(1, 2))(x)
