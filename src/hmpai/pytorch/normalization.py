...
def norm_mad_zscore(tensor: torch.Tensor, median: float, mad: float) -> torch.Tensor:
    return (tensor - median) / mad
...
def get_norm_vars_from_global_statistics(
    statistics: dict,
    normalization_fn: Callable[[torch.Tensor, float, float], torch.Tensor],
):
    if normalization_fn == norm_0_to_1:
        return statistics["global_min"], statistics["global_max"]
    elif normalization_fn == norm_min1_to_1:
        return statistics["global_min"], statistics["global_max"]
    elif normalization_fn == norm_zscore:
        return statistics["global_mean"], statistics["global_std"]
    elif normalization_fn == norm_mad_zscore:
         return statistics["global_median"], statistics["mad_score"]
    elif normalization_fn == norm_dummy:
        return
    else:
        return
