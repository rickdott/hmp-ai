...
def build_datasets(descriptors, val_size: float=0.5, test_size: float=0, montage_path: Path=Path('files/combined_montage.json')):
    ...
    for desc in descriptors:
        ...
        task_class_counts[strat] = len(desc["label"]) + 1 # Account for negative class
        rt_key = desc["rt_key"] if "rt_key" in desc else None
        print(desc)
        train.append(MultiXArrayProbaDataset(
            ...
            transform=Compose([RandomLowPassTransform(cutoffs=(30,), probability=0.8), StartJitterTransform(probability=1.0), EndJitterTransform(probability=1.0), ChannelShuffleTransform(1.0), ChannelDropoutTransform(1.0, 0.1)]),
...
class MultiXArrayProbaDataset(Dataset):
            ...
            for key in self.info_to_keep:
                if key in sample:
                    ...
                    if getattr(val, "item", None) is not None:
                        val = val.item()
                    sample_info[key] = val
            if "coords" in self.info_to_keep and self.channel_dict is not None:
    ...
    def close(self):
        """Close all open dataset file handles."""
